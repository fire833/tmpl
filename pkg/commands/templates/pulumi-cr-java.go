...
func NewPULUMIJAVACRCommand() *cobra.Command {
	type cmdOpts struct {
		Output *string
		Header *string
	}
	...
	cmd := &cobra.Command{
		Use:     "pulumi-cr-java",
		Aliases: []string{"pcrj"},
		Short:   "Generate boilerplate for creating new Pulumi Component Resources in java.",
		Long:    ``,
		Version: "0.0.1",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			tpl, tple := template.New("pulumi-cr-java").Funcs(sprig.TxtFuncMap()).Parse(tmpl)
			if tple != nil {
				return tple
			}
	...
	set := pflag.NewFlagSet("pulumi-cr-java", pflag.ExitOnError)
