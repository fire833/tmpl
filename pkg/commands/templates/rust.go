...
func NewRUSTCommand() *cobra.Command {
	type cmdOpts struct {
		output *string
	}
	...
	cmd := &cobra.Command{
		Use:     "rust",
		Aliases: []string{},
		Short:   "Generate boilerplate for creating new rust templates.",
		Long:    "",
		Version: "0.0.1",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			tpl, tple := template.New("rust").Funcs(sprig.TxtFuncMap()).Parse(tmpl)
			if tple != nil {
				return tple
			}
	...
	set := pflag.NewFlagSet("rust", pflag.ExitOnError)
